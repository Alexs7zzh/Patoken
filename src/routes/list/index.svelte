...
<div class="container">
	{#if editing === false}
					...
					<li id={item.id}>
						...
						<div class="actions">
							{#if $userStore && $userStore.name === item.author}
								<button on:click={() => edit(item.id)} aria-label="Edit this post">
								...
								<button on:click={() => del(item.id)} aria-label="Delete this post">
		...
		<button class="edit" on:click={startEdit} aria-label="Create a new post">
