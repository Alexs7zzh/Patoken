...
export function swr<T>(key: string | undefined, options: FetcherFn<T> | SWROptions<T> = {}): SWRResult<T> {
	...
	const { fetcher = (url: string) => fetch(url).then(r => r.json()), maxAge, plugins = [], updater } = options

	const store = getOrCreate<T>(key)
	const refresh = createRefresh(key, fetcher)

	const onSubscribe = readable(undefined, () => {
		if (!store.last_update || (maxAge !== undefined && Date.now() - maxAge > store.last_update)) {
			refresh()
		}
