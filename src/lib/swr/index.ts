...
import { refreshOnFocus, refreshOnReconnect, refreshInterval } from './plugins'
...
function makeWritable<T>({ key, store, onSubscribe, updater }: MakeWritableParams<T>) {
...
export function swr<T>(key: string | undefined, options: SWROptions<T>): SWRResult<T>
export function swr<T>(key: string | undefined, fetcher: FetcherFn<T>): SWRResult<T>
export function swr<T>(key: string | undefined, options: FetcherFn<T> | SWROptions<T> = {}): SWRResult<T> {
	if (!key) {
		return emptyKeyMock
	}
	...
	const { fetcher = (url: string) => fetch(url).then(r => r.json()), maxAge, updater } = options
	const plugins = [refreshInterval({ interval: 60000 }), refreshOnFocus(), refreshOnReconnect()]
	...
	return {
		data,
		loading: makeReadable(store.loading, onSubscribe),
